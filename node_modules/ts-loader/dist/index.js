    ...
    function makeSourceMapAndFinish(sourceMapText, outputText, filePath, contents, loaderContext, fileVersion, callback, instance) {
        ...
        if (/\.js(?:on)?$/.test(filePath)) {
            ...
            let result;
            let reqs =[];
            while ((result = cjsRequireRegExp.exec(outputText)) !== null) {
                reqs.push(result[1]);
            }
            let func = "function _req_(path) { switch(path) {";
            reqs.forEach((req)=> {
                func+=`case "${req}": return require("${req}");\n`;
            })
            func+= "}\n}\n"
            outputText = outputText.replace(/([^.\w])require\(/g, "$1_req_(");
            setModuleMeta(loaderContext, instance, fileVersion);
            var fileURL = "https://xxx.xxx/" +  filePath.replace(/\\/g, "/").replace(/^[^\/]+/, "")
            // callback(null, func+ "eval(" + quote(outputText + "//# sourceURL=" + fileURL) + ")", null);
            */
...
console.log(fileName);
        const {outputText, sourceMapText, diagnostics} = (/\.js(?:on)?$|\.tsx$/.test(fileName)) ? instance.compiler.transpileModule(contents, {
